package com.imagepicker.demo.style;
...
import com.imagepicker.data.ICameraExecutor;
import com.imagepicker.data.IReloadExecutor;
import com.imagepicker.demo.AlohaActivity;
...
import com.imagepicker.views.PickerUiProvider;
...
import com.imagepicker.demo.utils.ImagePickerHelper;
...
public class WeChatPresenter implements IPickerPresenter {
    private ImagePickerHelper.Config config;

    public WeChatPresenter(ImagePickerHelper.Config config) {
        this.config = config;
    }
    ...
    public boolean interceptPickerCompleteClick(Activity activity, ArrayList<ImageItem> selectedList, BaseSelectConfig selectConfig) {
        if (config.isAutoJumpAlohaActivity()) {
    ...
    public boolean interceptCameraClick(@Nullable final Activity activity, final ICameraExecutor takePhoto) {
        ...
        if (config != null) {
            if (config.getMimeType() == 0) {
                takePhoto.takePhoto();
            } else if (config.getMimeType() == 1) {
                takePhoto.takeVideo();
            }
        } else {
